...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // return $pdf->download('hariom.pdf');
        ...
        $featured_categories = Cache::rememberForever('featured_categories', function () {
            return Category::with('coverImage')->where('featured', 1)->get();
        });

        //   $featured_categories = Category::with('bannerImage', 'coverImage')->where('featured', 1)->get();


        return view('frontend.homepage', compact('featured_categories'));
    }
    ...
    public function membership()
    {
        ...
        $user = Auth::user();
        if (!empty($user)) {
            $membershipInCart = Cart::where(['user_id' => $user->id, 'product_id' => 28])->first();
            if (!empty($membershipInCart)) {
            ...
            $totalPoints = BsdkPoint::where('user_id', $user->id)->where('expires_at', '>', Carbon::now())->sum('points');
            $bsdkMoney = $totalPoints / 2;


            if (isset($user->member_expire_at) && now()->lessThan($user->member_expire_at)) {
                $memberShipStatus = 1;
            } else {
                $memberShipStatus = 0;
            }
        ...
        return view('frontend.membership', compact('user', 'totalPoints', 'bsdkMoney', 'memberShipStatus', 'membershipCartStatus'));
    }
    ...
    public function subCategory($slug)
    {
        ...
        $category = Category::with('bannerImage')->where('slug', $slug)->first();
        $subcategories = Category::with('catIcon', 'coverImage')->where('parent_id', $category->id)->where('theam', '!=', '1')->select('*')->get();
        $newArribles = Category::with('catIcon', 'coverImage')->where('parent_id', $category->id)->orderBy('id', 'desc')->select('*')->take(10)->get();
        $subcategoriesIds = Category::where('parent_id', $category->id)->pluck('id')->toArray();
        $shopByTheams = Category::with('coverImage')->where('parent_id', $category->id)->where('theam', '1')->select('*')->get();
        $topSellingProducts = Product::with('thumbnail')->whereIn('category_id', $subcategoriesIds)->where(['published' => '1', 'top_selling' => '1'])->orderBy('id', 'desc')->select('*')->get();

        return view('frontend.sbucategory', compact('category', 'subcategories', 'newArribles', 'topSellingProducts', 'shopByTheams'));
    }
    ...
    public function cart_login(Request $request)
    {
        ...
        if ($request->get('phone') != null) {
            ...
            $user = User::where(['user_type' => 'customer', 'email' => $request->email])->first();
        }
    ...
    public function product($slug)
    {
        ...
        $detailedProduct  = Product::with('reviews', 'brand',  'productTax', 'stocks', 'user', 'user.shop')->where('auction_product', 0)->where('slug', $slug)->where('approved', 1)->first();

        $allRating = Review::where(['product_id' => $detailedProduct->id, 'status' => '1'])->count();
        //one star rating percentage
        $oneStarRating = Review::where(['product_id' => $detailedProduct->id, 'status' => '1', 'rating' => '1'])->count();

        $totalOneRatingPer = $allRating != 0 ? ($oneStarRating * 100) / $allRating : 0;


        //two star rating percentage
        $twoStarRating = Review::where(['product_id' => $detailedProduct->id, 'status' => '1', 'rating' => '2'])->count();

        $totalTwoRatingPer = $allRating != 0 ? ($twoStarRating * 100) / $allRating : 0;

        //three star rating percentage
        $threeStarRating = Review::where(['product_id' => $detailedProduct->id, 'status' => '1', 'rating' => '1'])->count();

        $totalThreeRatingPer = $allRating != 0 ? ($threeStarRating * 100) / $allRating : 0;

        //four star rating percentage
        $fourStarRating = Review::where(['product_id' => $detailedProduct->id, 'status' => '1', 'rating' => '1'])->count();

        $totalFourRatingPer = $allRating != 0 ? ($fourStarRating * 100) / $allRating : 0;

        //five star rating percentage
        $fiveStarRating = Review::where(['product_id' => $detailedProduct->id, 'status' => '1', 'rating' => '1'])->count();

        $totalFiveRatingPer = $allRating != 0 ? ($fiveStarRating * 100) / $allRating : 0;

        // print_r($totalFiveRatingPer);die;




        $oneStarCount = Review::where(['product_id' => $detailedProduct->id, 'rating' => '1', 'status' => '1'])->count();

        $twoStarCount = Review::where(['product_id' => $detailedProduct->id, 'rating' => '2', 'status' => '1'])->count();
        $threeStarCount = Review::where(['product_id' => $detailedProduct->id, 'rating' => '3', 'status' => '1'])->count();

        $fourStarCount = Review::where(['product_id' => $detailedProduct->id, 'rating' => '4', 'status' => '1'])->count();

        $fiveStarCount = Review::where(['product_id' => $detailedProduct->id, 'rating' => '5', 'status' => '1'])->count();



        $reviews = Review::where(['product_id' => $detailedProduct->id, 'status' => '1'])->get();




        return view('frontend.productDetial', compact('detailedProduct', 'reviews', 'totalOneRatingPer', 'fiveStarCount', 'fourStarCount', 'threeStarCount', 'twoStarCount', 'oneStarCount', 'totalTwoRatingPer', 'totalThreeRatingPer', 'totalFourRatingPer', 'totalFiveRatingPer'));
    }



    public function productList($category, $subCategory)
    {
        ...
        // $msg91ApiUrl = 'https://control.msg91.com/api/v5/email/send';
        // $msg91ApiKey = '411853Ao5LCL0WCrX657c27dfP1';
        ...
        //     'recipients' => [
        //         [
        //             'to' => [['name' => 'Hariom', 'email' => 'hariompandeyp0@gmail.com']],
        //             'variables' => ['name' => 'Hariom'],
        //         ],

        //     ],
        //     'from' => ['name' => 'Joe', 'email' => 'hariompandeyp1@gmail.com'],
        //     'subject' => 'Test Email', // Added subject
        //     'text' => 'Your email content goes here.', // Added body content
        //     'domain' => 'bonkerzdonkerz.com',
        //     //'reply_to' => [['email' => 'reply1@email.com'], ['email' => 'reply2@email.com']],
        //     // 'attachments' => [
        //     //     ['filePath' => 'Public path for file.', 'fileName' => 'File Name'],
        //     //     ['file' => 'File in Data URI format data:content/type;base64,<data>.', 'fileName' => 'File Name'],
        //     // ],
        //     // 'template_id' => 'YOUR_UNIQUE_TEMPLATE_NAME',
        // ];

        // $response = $client->post($msg91ApiUrl, [
        //     'headers' => ['accept' => 'application/json', 'authkey' => $msg91ApiKey, 'content-type' => 'application/json'],
        //     'json' => $emailData,
        // ]);
        ...
        // // Prepare email data
        // $emailData = [
        //     'to' => 'hariompandeyp0@gmail.com',
        //     'from' => 'hariompandeyp1@gmail.com',
        ...
        // $response = $client->post($msg91ApiUrl, [
        //     'headers' => [
        //         'Authorization' => 'Bearer ' . $msg91ApiKey,
        ...
        // // Handle the response data or return a response to the client
        ...
        //       "to": [
        //         {
        //           "email": "bonkerzdonkerz@gmail.com",
        //           "name": "ARICK SHARMA"
        //         }
        //       ]
        //     }
        //   ],
        //   "from": {
        //     "email": "no-reply@sfe0zl.mailer91.com"
        //   },
        //   "domain": "sfe0zl.mailer91.com",

        // }',
        //     CURLOPT_HTTPHEADER => array(
        //         'Content-Type: application/json',
        //         'Accept: application/json',
        //         'authkey: {411853Ao5LCL0WCrX657c27dfP1}',
        //     ),
        // ));
        ...
        $parendId = $category->id;
        $subCategories = Category::where(['parent_id' => $category->id])->where('theam', '!=', '1')->select('*')->get();
        $theams = Category::where(['parent_id' => $category->id])->where('theam', '1')->select('*')->get();

        $subCategory = Category::where(['parent_id' => $category->id, 'slug' => $subCategory])->first();

        $products = Product::with('thumbnail')->where('category_id', $subCategory->id)->where(['published' => '1',])->orderBy('id', 'desc')->select('*')->paginate(12);

        $sizes = AttributeValue::where('attribute_id', '1')->get();
        $colors = Color::orderBy('name')->take(10)->get();
        return view('frontend.product_list', compact('products', 'subCategories', 'theams', 'sizes', 'colors', 'parendId'));
    }

    public function short(Request $request)
    {

        $explodedArray = explode('-', $request->dropdown_filter);


        $parendId = $explodedArray[1];
        $value = $explodedArray[0];




        $subCategories = Category::where(['parent_id' => $parendId])->where('theam', '!=', '1')->select('*')->get();

        $theams = Category::where(['parent_id' => $parendId])->where('theam', '1')->select('*')->get();

        $subCategory = Category::where(['parent_id' => $parendId])->pluck('id')->toArray();



        if ($value == 'asc' || $value = 'desc') {

            $products = Product::with('thumbnail')->whereIn('category_id', $subCategory)->where(['published' => '1',])->orderBy('unit_price', $value)->select('*')->paginate(12);
        } else {
            $products = Product::with('thumbnail')->whereIn('category_id', $subCategory)->where(['published' => '1',])->orderBy('id', 'desc')->select('*')->paginate(12);
        }
        $sizes = AttributeValue::where('attribute_id', '1')->get();
        $colors = Color::orderBy('name')->take(10)->get();
    ...
    {
        $query = Product::with('thumbnail')->where(['published' => '1']);
        if (!empty($request->category_ids)) {



            $query->whereIn('category_id', $request->category_ids);
        } elseif (!empty($request->parendId)) {
            $categoryIds = Category::where('parent_id', $request->parendId)->pluck('id')->toArray();
        ...
        if (!empty($request->sizes)) {
            ...
            $query->whereJsonContains('choice_options', [
                'attribute_id' => '1',
                'values' => $request->sizes,
            ]);
        }

        $products = $query->orderBy('id', 'desc')->select('*')->get();
        ...
        // $products = $filteredData = Product::->get();




        return view('frontend.partial.listproduct', compact('products'));
    }
    ...
    public function top_10_settings(Request $request)
    {
        foreach (Category::all() as $key => $category) {
            if (is_array($request->top_categories) && in_array($preantId, $request->top_categories)) {
                $category->top = 1;
                $category->save();
            } else {
                $category->top = 0;
