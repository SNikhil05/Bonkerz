...
class PurchaseHistoryController extends Controller
{
    ...
    {
        $orders = Order::with('orderDetails')->where('user_id', Auth::user()->id)->orderBy('code', 'desc')->get();
        $user = Auth::user();
        
        return view('frontend.order_history', compact('orders','user'));
    }
    ...
    public function purchase_history_details($id)
    {
        ...
        $order->save();
        $user = Auth::user();
        return view('frontend.orderDetial', compact('order', 'user'));
    }
