        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th data-breakpoints="lg">Theam</th>
                ...
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td>
                         ...
                         <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" onchange="update_theam(this)" value="{{ $category->id }}" <?php if($category->theam == 1) echo "checked";?>>
                                <span></span>
                            </label>
                        </td>
    ...
    <script type="text/javascript">
        function update_featured(el){
           ...
           function update_theam(el){
            ...
            $.post('{{ route('categories.theam') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Theam categories updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
